...
const FORM_VACIO = {
  ...
  medios_verificacion: "", supuestos: "", dependency_id: "", strategy_id: ""
}
...
export default function FichasTecnicas({ dependencias = [] }) {
  ...
  // --- ESTADOS DE ESTRATEGIAS (Corregidos) ---
  ...
  // --- LÓGICA DE DEPENDENCIAS Y ESTRATEGIAS (Corregida) ---
  const handleDependenciaChange = async (e) => {
    ...
    try {
      ...
      setEstrategiasDep(data)
    } catch (e) {
      console.error("Error al cargar estrategias", e)
    } finally {
  ...
  const handleEstrategiaChange = (e) => {
    ...
    const sel = estrategiasDep.find(s => String(s.strategy_id) === String(selId))
    if (!sel) return
    setForm(prev => ({
      ...prev,
      ...
      eje: sel.eje || "",
      tema: sel.tema || "",
      politica_publica: sel.politica_publica || "",
  ...
  const inputStyle = { width: "100%", padding: "8px 10px", borderRadius: "6px", border: "1px solid #d1d5db", fontSize: "13px", boxSizing: "border-box", color: "#000000", background: "#ffffff" }
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
            ...
            <div style={{ gridColumn: "1/-1" }}>
              ...
              <select name="dependency_id" value={form.dependency_id} onChange={handleDependenciaChange} style={inputStyle}>
                <option value="" style={{color: "#000"}}>Selecciona una dependencia</option>
                {dependencias.map(d => (
            ...
            {form.dependency_id && (
              <div style={{ gridColumn: "1/-1" }}>
                <label style={labelStyle}>
                  Estrategia *
                  {cargandoEst && <span style={{ fontWeight: "400", color: "#6b7280", marginLeft: "8px" }}>Cargando...</span>}
                </label>
                <select
                ...
                >
                  <option value="" style={{color: "#000"}}>
                    {cargandoEst ? "Cargando estrategias..." : estrategiasDep.length === 0 ? "Sin estrategias disponibles" : "Selecciona una estrategia"}
                  </option>
                  {estrategiasDep.map(s => (
                    <option key={s.strategy_id} value={s.strategy_id} style={{color: "#000"}}>
                      {s.estrategia}
                    </option>
                  ))}
                </select>
            ...
            {form.estrategia && (
              <div style={{ gridColumn: "1/-1", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: "8px", padding: "14px 16px" }}>
                <p style={{ fontWeight: "700", fontSize: "12px", color: "#166534", marginBottom: "10px" }}>
                  ✅ Datos auto-rellenados desde la base de datos
                </p>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                  {[
                    { label: "Eje", value: form.eje },
                    { label: "Tema", value: form.tema },
                    { label: "Política Pública", value: form.politica_publica },
                    { label: "Objetivo", value: form.objetivo },
                  ].map(({ label, value }) => (
                    <div key={label}>
                      <p style={{ fontSize: "11px", fontWeight: "700", color: "#374151", margin: "0 0 2px" }}>{label}</p>
                      <p style={{ fontSize: "12px", color: "#000000", margin: 0, background: "white", padding: "6px 8px", borderRadius: "4px", border: "1px solid #e5e7eb" }}>
                        {value || "-"}
                      </p>
                    </div>
                  ))}
