...
export default function TransparenciaSeccion5() {
  ...
  const cargar = async () => {
    setCargando(true)
    try {
      const res = await fetch("http://localhost:3000/api/transparencia/seccion5")
  ...
  const handleGuardar = async () => {
    ...
    try {
      const url = editando
        ? `http://localhost:3000/api/transparencia/seccion5/${editando}`
        : "http://localhost:3000/api/transparencia/seccion5"
  ...
  const handleEliminar = async (id) => {
    ...
    await fetch(`http://localhost:3000/api/transparencia/seccion5/${id}`, { method:"DELETE" })
