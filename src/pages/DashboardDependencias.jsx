...
export default function DashboardDependencias() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3000/api/auth/me", { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(data => {
        ...
        fetch(`http://localhost:3000/api/pdf/habilitados/${data.dependency_id}`)
          .then(r => r.json())
  ...
  const seleccionarEstrategia = async (e) => {
    ...
    const lineasIniciales = await Promise.all(e.lineas.map(async (l) => {
      const res = await fetch(`http://localhost:3000/api/trimestres/porLinea/${l.id}`);
    ...
    try {
      for (const l of lineas) {
        ...
        for (const r of registros) {
          await fetch("http://localhost:3000/api/trimestres/guardar", {
  ...
  const enviarRevision = async () => {
    ...
    const res = await fetch("http://localhost:3000/api/lineas/nueva", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ estrategia_id: selectedStrategy.id, lineas_accion: nuevaLinea }) });
