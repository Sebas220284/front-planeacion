...
export default function DashboardEstrategica() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3100/api/pmd/lista")
      .then(r => r.json())
      .then(setPlanes)
  ...
  const handleEnviar = async () => {
    ...
    setEnviando(true)
    try {
      const res = await fetch("http://localhost:3100/api/pmd/crear", {
        method: "POST",
  ...
  const handleRevisar = async (estado) => {
    ...
    await fetch(`http://localhost:3100/api/pmd/revisar/${modalRevisar.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
